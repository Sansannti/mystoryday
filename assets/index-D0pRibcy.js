var E=a=>{throw TypeError(a)};var I=(a,e,t)=>e.has(a)||E("Cannot "+t);var l=(a,e,t)=>(I(a,e,"read from private field"),t?t.call(a):e.get(a)),y=(a,e,t)=>e.has(a)?E("Cannot add the same private member more than once"):e instanceof WeakSet?e.add(a):e.set(a,t),f=(a,e,t,i)=>(I(a,e,"write to private field"),i?i.call(a,t):e.set(a,t),t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))i(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const n of s.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function t(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(r){if(r.ep)return;r.ep=!0;const s=t(r);fetch(r.href,s)}})();const b="https://story-api.dicoding.dev/v1";class w{static async register(e,t,i){const r=await fetch(`${b}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,email:t,password:i})}),s=await r.json();if(!r.ok)throw new Error(s.message||"Registration failed");return s}static async login(e,t){const i=await fetch(`${b}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t})}),r=await i.json();if(!i.ok)throw new Error(r.message||"Login failed");return r}static async getStories(){const e=localStorage.getItem("token");if(!e)throw new Error("No token found. Please login first.");const t=await fetch(`${b}/stories`,{headers:{Authorization:`Bearer ${e}`}}),i=await t.json();if(!t.ok)throw new Error(i.message||"Failed to fetch stories");return i}static async addStory(e,t,i,r){const s=localStorage.getItem("token");if(!s)throw new Error("No token found. Please login first.");const n=new FormData;n.append("description",e),n.append("photo",t),i&&r&&(n.append("lat",i),n.append("lon",r));const o=await fetch(`${b}/stories`,{method:"POST",headers:{Authorization:`Bearer ${s}`},body:n}),c=await o.json();if(!o.ok)throw new Error(c.message||"Failed to add story");return c}}class v{static saveToken(e){localStorage.setItem("token",e)}static getToken(){return localStorage.getItem("token")}static removeToken(){localStorage.removeItem("token")}static isLoggedIn(){return!!this.getToken()}static saveUserInfo(e,t){localStorage.setItem("userName",e),localStorage.setItem("userId",t)}static getUserName(){return localStorage.getItem("userName")}static getUserId(){return localStorage.getItem("userId")}static clearUserInfo(){localStorage.removeItem("userName"),localStorage.removeItem("userId")}static logout(){this.removeToken(),this.clearUserInfo()}}const B={async render(){return`
    ...
    `},async afterRender(){const a=document.getElementById("registerForm"),e=document.getElementById("registerAlert");a.addEventListener("submit",async t=>{t.preventDefault();const i=document.getElementById("name").value,r=document.getElementById("email").value,s=document.getElementById("password").value;if(s.length<8){e.innerHTML='<div class="alert alert-error">Password minimal 8 karakter</div>';return}try{e.innerHTML='<div class="alert alert-success">Loading...</div>',await w.register(i,r,s),e.innerHTML='<div class="alert alert-success">Registrasi berhasil! Redirecting to login...</div>',setTimeout(()=>{window.location.hash="#/login"},1500)}catch(n){e.innerHTML=`<div class="alert alert-error">${n.message}</div>`}})}},F="MyStoryDayDB",D=1,d="favorites",u="pendingStories";class g{static async openDB(){return new Promise((e,t)=>{const i=indexedDB.open(F,D);i.onerror=()=>{t(new Error("Failed to open database"))},i.onsuccess=()=>{e(i.result)},i.onupgradeneeded=r=>{const s=r.target.result;s.objectStoreNames.contains(d)||s.createObjectStore(d,{keyPath:"id"}).createIndex("createdAt","createdAt",{unique:!1}),s.objectStoreNames.contains(u)||s.createObjectStore(u,{keyPath:"id",autoIncrement:!0}).createIndex("timestamp","timestamp",{unique:!1})}})}static async addFavorite(e){try{const r=(await this.openDB()).transaction([d],"readwrite").objectStore(d),s={...e,savedAt:new Date().toISOString()};return await r.add(s),!0}catch(t){return console.error("Error adding favorite:",t),!1}}static async removeFavorite(e){try{return await(await this.openDB()).transaction([d],"readwrite").objectStore(d).delete(e),!0}catch(t){return console.error("Error removing favorite:",t),!1}}static async getAllFavorites(){try{const i=(await this.openDB()).transaction([d],"readonly").objectStore(d);return new Promise((r,s)=>{const n=i.getAll();n.onsuccess=()=>r(n.result),n.onerror=()=>s(n.error)})}catch(e){return console.error("Error getting favorites:",e),[]}}static async isFavorite(e){try{const r=(await this.openDB()).transaction([d],"readonly").objectStore(d);return new Promise((s,n)=>{const o=r.get(e);o.onsuccess=()=>s(!!o.result),o.onerror=()=>n(o.error)})}catch(t){return console.error("Error checking favorite:",t),!1}}static async searchFavorites(e){const t=await this.getAllFavorites(),i=e.toLowerCase();return t.filter(r=>r.name.toLowerCase().includes(i)||r.description.toLowerCase().includes(i))}static async sortFavorites(e="newest"){const t=await this.getAllFavorites();switch(e){case"newest":return t.sort((i,r)=>new Date(r.savedAt)-new Date(i.savedAt));case"oldest":return t.sort((i,r)=>new Date(i.savedAt)-new Date(r.savedAt));case"name":return t.sort((i,r)=>i.name.localeCompare(r.name));default:return t}}static async addPendingStory(e){try{const r=(await this.openDB()).transaction([u],"readwrite").objectStore(u),s={...e,timestamp:new Date().toISOString(),synced:!1};return new Promise((n,o)=>{const c=r.add(s);c.onsuccess=()=>n(c.result),c.onerror=()=>o(c.error)})}catch(t){throw console.error("Error adding pending story:",t),t}}static async getPendingStories(){try{const i=(await this.openDB()).transaction([u],"readonly").objectStore(u);return new Promise((r,s)=>{const n=i.getAll();n.onsuccess=()=>r(n.result),n.onerror=()=>s(n.error)})}catch(e){return console.error("Error getting pending stories:",e),[]}}static async deletePendingStory(e){try{const r=(await this.openDB()).transaction([u],"readwrite").objectStore(u);return new Promise((s,n)=>{const o=r.delete(e);o.onsuccess=()=>s(!0),o.onerror=()=>n(o.error)})}catch(t){return console.error("Error deleting pending story:",t),!1}}static async clearPendingStories(){try{const i=(await this.openDB()).transaction([u],"readwrite").objectStore(u);return new Promise((r,s)=>{const n=i.clear();n.onsuccess=()=>r(!0),n.onerror=()=>s(n.error)})}catch(e){return console.error("Error clearing pending stories:",e),!1}}}const p={VAPID_PUBLIC_KEY:"BCCs2eonMI-6H2ctvFaWg-UYdDv387Vno_bzUzALpB442r2lCnsHmtrx8biyPi_E-1fSGABK_Qs_GlvPoJJqxbk",API_BASE_URL:"https://story-api.dicoding.dev/v1",_urlBase64ToUint8Array(a){const e="=".repeat((4-a.length%4)%4),t=(a+e).replace(/-/g,"+").replace(/_/g,"/"),i=window.atob(t),r=new Uint8Array(i.length);for(let s=0;s<i.length;++s)r[s]=i.charCodeAt(s);return r},async requestPermission(){if(!("Notification"in window))return console.warn("This browser does not support notifications"),!1;try{const a=await Notification.requestPermission();return console.log("Notification permission:",a),a==="granted"}catch(a){return console.error("Error requesting notification permission:",a),!1}},async isSubscribed(){if(!("serviceWorker"in navigator)||!("PushManager"in window))return!1;try{return await(await navigator.serviceWorker.ready).pushManager.getSubscription()!==null}catch(a){return console.error("Error checking subscription:",a),!1}},async subscribe(){if(!("serviceWorker"in navigator)||!("PushManager"in window))throw new Error("Push not supported");const a=v.getToken();if(!a)throw new Error("Token tidak ditemukan");const e=await navigator.serviceWorker.ready;let t=await e.pushManager.getSubscription();if(!t){const s=this._urlBase64ToUint8Array(this.VAPID_PUBLIC_KEY);t=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:s})}const i=t.toJSON(),r=await fetch("https://story-api.dicoding.dev/v1/notifications/subscribe",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`},body:JSON.stringify({endpoint:i.endpoint,keys:{p256dh:i.keys.p256dh,auth:i.keys.auth}})});if(!r.ok){const s=await r.json();throw new Error(s.message)}return t},async unsubscribe(){if(!("serviceWorker"in navigator)||!("PushManager"in window))return!1;const a=v.getToken();if(!a)return console.warn("No auth token - cannot unsubscribe from server"),!1;try{const t=await(await navigator.serviceWorker.ready).pushManager.getSubscription();if(!t)return console.log("No active subscription to unsubscribe"),!0;console.log("Unsubscribing from push notifications...");const r=t.toJSON().endpoint;console.log("Endpoint to unsubscribe:",r);const s=await fetch(`${this.API_BASE_URL}/notifications/subscribe`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${a}`},body:JSON.stringify({endpoint:r})});if(!s.ok){const c=await s.json();throw console.error("Server unsubscribe error:",c),new Error(c.message||"Failed to unsubscribe from server")}const n=await s.json();console.log("✅ Server unsubscribe successful:",n);const o=await t.unsubscribe();return console.log("✅ Local unsubscribe successful:",o),o}catch(e){console.error("❌ Error unsubscribing:",e);try{const i=await(await navigator.serviceWorker.ready).pushManager.getSubscription();if(i){const r=await i.unsubscribe();return console.log("⚠️ Local unsubscribe fallback successful:",r),r}}catch(t){console.error("❌ Local unsubscribe also failed:",t)}throw e}},async showTestNotification(){"serviceWorker"in navigator&&await(await navigator.serviceWorker.ready).showNotification("MyStoryDay",{body:"Push notifications berhasil diaktifkan! 🎉",icon:"/mystoryday/icon-192x192.png",badge:"/mystoryday/icon-96x96.png",vibrate:[200,100,200],data:{url:"/mystoryday/#/stories"}})},async showFavoriteNotification(a){if(!(!("serviceWorker"in navigator)||Notification.permission!=="granted"))try{await(await navigator.serviceWorker.ready).showNotification("Cerita Ditambahkan ke Favorit! ❤️",{body:`"${a}" telah disimpan ke favorit Anda`,icon:"/mystoryday/icon-192x192.png",badge:"/mystoryday/icon-96x96.png",vibrate:[200,100,200],data:{url:"/mystoryday/#/favorites"},actions:[{action:"view",title:"Lihat Favorit"},{action:"close",title:"Tutup"}]})}catch(t){console.error("Error showing favorite notification:",t)}},async showUnfavoriteNotification(a){if(!(!("serviceWorker"in navigator)||Notification.permission!=="granted"))try{await(await navigator.serviceWorker.ready).showNotification("Cerita Dihapus dari Favorit",{body:`"${a}" telah dihapus dari favorit Anda`,icon:"/mystoryday/icon-192x192.png",badge:"/mystoryday/icon-96x96.png",vibrate:[200],data:{url:"/mystoryday/#/favorites"}})}catch(t){console.error("Error showing unfavorite notification:",t)}}},C={_stories:[],_map:null,_markers:[],_currentLayer:"streets",_favoriteIds:new Set,async render(){return`
       <div class="container">
    ...
    `},async afterRender(){this._initMap(),this._setupFileUpload(),this._setupCamera(),this._setupForm(),this._setupConnectionMonitor(),this._checkPendingStories()},_setupConnectionMonitor(){const a=document.getElementById("connectionStatus");window.addEventListener("online",()=>{a.innerHTML='<span class="status-online">🟢 Online</span>',this._syncPendingStories()}),window.addEventListener("offline",()=>{a.innerHTML='<span class="status-offline">🔴 Offline - Cerita akan dikirim saat online</span>'})},async _checkPendingStories(){try{const a=await g.getPendingStories(),e=document.getElementById("pendingStoriesStatus");if(a.length>0&&(e.style.display="block",e.innerHTML=`
        ...
        `,navigator.onLine)){const t=document.getElementById("syncNowBtn");t&&t.addEventListener("click",()=>this._syncPendingStories())}}catch(a){console.error("Error checking pending stories:",a)}},async _syncPendingStories(){const a=document.getElementById("pendingStoriesStatus"),e=document.getElementById("addStoryAlert");try{const t=await g.getPendingStories();if(t.length===0)return;a.innerHTML='<div class="alert alert-success">⏳ Mengirim cerita yang tertunda...</div>';let i=0;for(const s of t)try{await w.addStory(s.description,s.photo,s.lat,s.lon),await g.deletePendingStory(s.id),i++}catch(n){console.error("Error syncing story:",n)}if(i>0&&(a.innerHTML=`<div class="alert alert-success">✅ ${i} cerita berhasil dikirim!</div>`,setTimeout(()=>{a.style.display="none"},3e3)),!("serviceWorker"in navigator)){console.warn("Service Worker not supported");return}const r=await navigator.serviceWorker.ready;if(!r){console.warn("SW registration not ready");return}if(!("sync"in r)){console.warn("Background Sync not supported");return}await r.sync.register("sync-stories")}catch(t){console.error("Error syncing stories:",t),e.innerHTML=`<div class="alert alert-error">Error syncing stories: ${t.message}</div>`}},_initMap(){this._map&&(this._map.remove(),this._map=null,this._markers=[]),this._map=L.map("addStoryMap").setView([-6.2088,106.8456],10),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap contributors"}).addTo(this._map),this._map.on("click",a=>{const{lat:e,lng:t}=a.latlng;this._marker&&this._map.removeLayer(this._marker),this._marker=L.marker([e,t]).addTo(this._map),this._selectedLocation={lat:e,lon:t},document.getElementById("coordsDisplay").textContent=`${e.toFixed(6)}, ${t.toFixed(6)}`})},_setupFileUpload(){const a=document.getElementById("photo"),e=document.getElementById("filePreview"),t=document.getElementById("cameraSection");a.addEventListener("change",i=>{const r=i.target.files[0];if(r){this._selectedFile=r;const s=new FileReader;s.onload=n=>{e.innerHTML=`
 ...
 <div class="container">
    ...
    `},async afterRender(){await this._loadFavorites(),this._setupSearchAndSort()},async _loadFavorites(){const a=document.getElementById("favoritesGrid"),e=document.getElementById("emptyState"),t=document.getElementById("favoritesAlert");try{if(this._favorites=await g.getAllFavorites(),this._filteredFavorites=[...this._favorites],this._favorites.length===0){a.style.display="none",e.style.display="flex";return}this._renderFavorites()}catch(i){t.innerHTML=`<div class="alert alert-error">Error loading favorites: ${i.message}</div>`,a.innerHTML=""}},_renderFavorites(){const a=document.getElementById("favoritesGrid"),e=document.getElementById("emptyState");if(this._filteredFavorites.length===0){a.innerHTML=`
